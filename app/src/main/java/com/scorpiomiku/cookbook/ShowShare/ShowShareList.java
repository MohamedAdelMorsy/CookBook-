package com.scorpiomiku.cookbook.ShowShare;
...
import com.scorpiomiku.cookbook.ShowShare.SSadapter.SSAdapter;
...
public class ShowShareList extends AppCompatActivity {
    ...
    protected void onCreate( Bundle savedInstanceState) {
        ...
        Log.d("分享界面", " 导入布局");
        setContentView(R.layout.show_share_list);
        ...
        ssecCL.findObjects(new FindListener<Post>() {
            ...
            public void done(List<Post> object, BmobException e) {
                if(e==null){
                    //问题所在
                    Log.d("分享界面，成功", "普通模式：测试点:size = "+ object.size());
                    int i= 0;
                    for (;i<object.size();i++){
                        Log.d("分享界面，成功", "普通模式：测试点:i设置成功");
                        final Post rt = object.get(i);
                        Log.d("分享界面，成功", "普通模式：测试点:i 设置成功");
                        Log.d("分享界面，成功", "普通模式：测试点:"+rt.getObjID());
                        Log.d("分享界面，成功", "普通模式：测试点:评论内容是: "+rt.getContent());
                        final String objid=rt.getAuthor().getObjectId();
                        BmobQuery<Person> query = new BmobQuery<Person>();
                        query.getObject(objid, new QueryListener<Person>() {

                            @Override
                            public void done(Person object, BmobException e) {
                                if(e==null){
                                    //获得person的昵称
                                    SSEC ssec = new SSEC(object.getUsername(),rt.getContent(),object.getObjectId());
                                    mssec.add(ssec);
                                }else{
                        ...
                        Log.d("分享界面，成功", "普通模式：测试点:评论人: "+rt.getAuthor().toString());
                        Log.d("分享界面，成功", "普通模式：测试点:评论人昵称: "+rt.getAuthor().getUsername());
                        Log.d("分享界面，成功", "普通模式：测试点:评论人email: "+rt.getAuthor().getEmail());
                        initWay(rt.getObjID());
                    }
                }else {
                    Log.d("分享界面，失败", "普通模式：测试点:"+object.toString());
                    Log.d("分享界面，失败", "普通模式：测试点:"+e.getMessage());
                }
            }
        });
        Log.d("分享界面加载", "onCreate: 检查点"+"总布局加载成功");
        recyclerView = (RecyclerView) findViewById(R.id.ssl_recycler_pl_all);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
         ssAdapter = new SSAdapter(this, mCookBookList);
//        businessAdapter.setOnRecyclerViewListener(this);
        Log.d("测试代码部分", "onCreate: 检查点"+"加入ada完成");
        ssAdapter.setOnRecyclerViewListener(new SSAdapter.OnRecyclerViewListener() {
            @Override
            public void onPLImgClick(View view, int position) {
                                                    ...
                                                    for (int i = 0; i <= array.size(); i++) {
                                                        ...
                                                        new Thread(new Runnable() {
                                                            ...
                                                            public void run() {
                                                                ...
                                                                mCookBookList.add(new CBLEC(ZuoFaMing, ZuoFaTu, "这里是介绍", Way_objectID,FromUser,mssec));
                                                                //*/
                                                                Log.d("initWay", "测试点5"+"放入对象成功");
                                                                recyclerView.post(new Runnable() {
                                                                    @Override
                                                                    public void run() {
                                                                        ssAdapter.setSSAdapter(mCookBookList);
    ...
    private Drawable loadImageFromNetwork(String urladdr, String Imagename) {
